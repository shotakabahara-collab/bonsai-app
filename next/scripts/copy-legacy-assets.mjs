...
if (!html.includes('BONSAI') || !html.includes('photo-assets.js')) {
  throw new Error('Built shell is incomplete');
}
const bridge = await readFile(photoTarget, 'utf8');
if (!bridge.includes('BonsaiPhotos')) throw new Error('photo-assets.js bridge is invalid');
const info = await stat(photoTarget);
if (info.size < 100) throw new Error('photo-assets.js bridge is too small');
console.log(`BONSAI build ready: ${info.size} byte local photo asset bridge`);
